...
NMClaw 和 [OpenClaw](https://github.com/openclaw/openclaw) 都是开源 AI 助手平台，但设计哲学完全不同：OpenClaw 是面向个人的全渠道消息助手（单 Agent + 多渠道），NMClaw 是面向团队/企业的多 Agent 编排内核（多 Agent + 智能调度）。v2.2 后 NMClaw 已补齐渠道和浏览器短板。
...
| **MCP 工具** | stdio / SSE + 内置 20+ 工具 + 浏览器控制 + Agent 级隔离 | 内置工具（browser/canvas/nodes/cron/sessions） |
| **记忆系统** | 4 层记忆栈 + 5 类自动提取 + 宫殿结构 + 正向最大匹配中文分词 + 语义搜索 | Session pruning（上下文裁剪） |
| **知识图谱** | ✅ 时序实体-关系三元组，valid_from/valid_to 时间窗口 | ❌ |
| **实体提取** | ✅ 中文人名（80+ 姓氏）+ 引号/书名号 + 中英混合术语 + 50+ 情感词 | ❌ |
...
| **IM 渠道** | ✅ 7 个：飞书 + Telegram + Discord + Slack + 企业微信 + 钉钉 + 微信公众号 | 24 个渠道（WhatsApp/Telegram/Slack/Discord/Signal/iMessage/Teams/Matrix/微信 等） |
| **企业微信** | ✅ 智能机器人长连接（wss://openws.work.weixin.qq.com，Bot ID + Secret，心跳保活 + 断线重连） | ✅ 企微插件（@wecom/wecom-openclaw-cli） |
| **语音** | ❌ | ✅ Voice Wake + Talk Mode（macOS/iOS/Android） |
| **Canvas** | ❌ | ✅ A2UI agent 驱动的可视化工作区 |
| **浏览器控制** | ✅ Playwright 14 个内置工具（导航/截图/快照/点击/输入/JS执行/标签页/PDF） | ✅ 专用 Chrome/Chromium CDP 控制 |
| **移动端** | ❌ | ✅ iOS + Android 原生 App |
| **桌面端** | ❌ | ✅ macOS 菜单栏 App |
| **Web 控制台** | ✅ React 19 + Tailwind 暗色主题（12 个页面，7 渠道配置 UI） | ✅ Control UI + WebChat + Dashboard |
...
| **部署** | Node.js 单进程，pnpm install 自动装 Chromium | Node.js + launchd/systemd daemon，Docker/Nix/Tailscale |
| **沙箱** | PTC 安全沙箱（环境变量过滤 + 工具白名单） | Docker per-session 沙箱（非 main session） |
| **技术栈** | TypeScript + Hono + React 19 + better-sqlite3 + playwright-core + grammy + discord.js + @slack/bolt | TypeScript + Gateway WS + Pi agent RPC |
| **存储** | SQLite（本地，零依赖） | 文件系统 + SQLite |
| **开源协议** | MIT | MIT |

**总结：** v2.2 后 NMClaw 已补齐渠道（7 个 IM + 企微智能机器人长连接）和浏览器控制（Playwright 14 工具）短板。OpenClaw 仍在语音交互、移动端 App、渠道数量（24 vs 7）上领先；NMClaw 在多 Agent 编排、智能记忆（4 层栈 + 中文分词 + 知识图谱）、工具并发、技能进化、安全扫描上领先。选 OpenClaw 做个人全渠道助手，选 NMClaw 做多 Agent 协作平台。
